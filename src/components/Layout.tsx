    ...
    <div className="min-h-screen flex flex-col font-sans bg-background text-primary">
      ...
      <header className="fixed top-0 left-0 right-0 z-40 bg-background/80 backdrop-blur-md border-b border-border no-print">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            ...
            <NavLink
              ...
              to={link.path}
              className={({ isActive }) =>
              `text-sm font-mono transition-colors flex items-center gap-2 ${isActive ? 'text-accent' : 'text-secondary hover:text-primary'}`
              }>
        ...
        <div className="md:hidden absolute top-16 left-0 right-0 bg-surface border-b border-border px-6 py-4 flex flex-col gap-4 shadow-lg">
          ...
          <NavLink
            ...
            to={link.path}
            className={({ isActive }) =>
            `text-base font-mono transition-colors flex items-center gap-3 ${isActive ? 'text-accent' : 'text-secondary'}`
            }>
      ...
      <footer className="border-t border-border py-12 mt-24 no-print">
        ...
        <div className="max-w-6xl mx-auto px-6 mt-8 text-center md:text-left text-xs text-secondary font-mono">
          &copy; {new Date().getFullYear()} Vallabh Kulkarni. Built with React &
          Tailwind.
        </div>
      </footer>
    </div>);
